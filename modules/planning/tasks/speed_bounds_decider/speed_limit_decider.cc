...
Status SpeedLimitDecider::GetSpeedLimits(
    ...
    for (uint32_t i = 0; i < discretized_path.size(); ++i) {
        ...
        const auto& referencew_ove = reference_line_.map_path().junction_overlaps();
        for (const auto& ove : referencew_ove) {
            if (std::fabs(ove.start_s - reference_line_s) <= 30) {
                speed_limit_data->AppendSpeedLimit(path_s, 4.5);
                // return Status::OK();
                break;
            }
        }
        ...
        print_curve.AddPoint("speed_limit_from_ref", path_s, speed_limit_from_reference_line);
        ...
        print_curve.AddPoint("speed_limit_from_centripetal_acc", path_s, speed_limit_from_centripetal_acc);
        ...
        for (const auto* ptr_obstacle : obstacles.Items()) {
            int num = 0;
            num = obstacles.Items().size();
            if (num >= 13) {
                speed_limit_data->AppendSpeedLimit(path_s, 7);
                break;
            }
            ...
            if (vehicle_front_s < obstacle_back_s || vehicle_back_s > obstacle_front_s) {
                continue;
            }
        ...
        if (speed_limit_from_nearby_obstacles < std::numeric_limits<double>::max()) {
            print_curve.AddPoint("speed_limit_from_nearby_obstacles", path_s, speed_limit_from_nearby_obstacles);
        }
